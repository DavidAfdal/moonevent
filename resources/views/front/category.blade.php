...
@section('content')
    <section id="content" class="max-w-[640px] w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
        ...
        <div class="flex flex-col gap-3 px-4">
            @if($category->tours && $category->tours->isNotEmpty())
                        ...
                        <div class="bg-white p-4 rounded-[26px] flex flex-col gap-3">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <p class="text-sm leading-[22px] tracking-035">
                                    <span class="font-semibold text-[#4D73FF] text-nowrap">Rp {{ number_format($tour->price, 0, ',', ',') }}</span>
                                    <span class="text-darkGrey">{{ $tour->days }} session</span>
                                </p>
                            </div>
                        </div>
                    </a>
            @empty
                <p>Belum ada paket wedding pada kategori ini</p>
            @endforelse
            @endif
        </div>
    </section>
